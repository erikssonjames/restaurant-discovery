...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const { restaurants, cities, cuisines } =
  ...
  const cityPages: MetadataRoute.Sitemap = cities.map(
    (city) => ({
      url: absoluteUrl(
        `/restaurants/city/${city.slug}`,
      ),
  ...
  const cuisinePages: MetadataRoute.Sitemap =
    cuisines.map((cuisine) => ({
      url: absoluteUrl(
        `/restaurants/cuisine/${cuisine.slug}`,
      ),
